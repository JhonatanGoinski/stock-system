...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalCost = sales.reduce((sum, sale) => sum + Number(sale.product.costPrice) * sale.quantity, 0)
    ...
    const formattedSales = sales.map((sale) => ({
      ...
      profit: Number(sale.totalAmount) - Number(sale.product.costPrice) * sale.quantity,
      notes: sale.notes,
    }))
    ...
    const reportData = {
      ...
      customerStats: customerStatsWithDetails.sort((a, b) => b.totalSpent - a.totalSpent),
      productStats: productStatsWithDetails.sort((a, b) => b.totalRevenue - a.totalRevenue),
    }
